...
import { ChevronRight, Shield, Feather, Sparkles, Image as ImageIcon, Plus, Trash2, Loader2 } from "lucide-react";
import TransliteratedTextArea from "@/components/diary/TransliteratedTextArea";
import { uploadToCloudinary } from "@/lib/cloudinary";
import { useToast } from "@/context/ToastContext";

type Mood = "joy" | "peace" | "love" | "reflection" | "neutral";
...
const MOODS: { key: Mood; label: string; emoji: string; color: string; glow: string }[] = [
    { key: "joy", label: "Joy", emoji: "✨", color: "bg-joy/20 border-joy/50 text-joy", glow: "shadow-[0_0_20px_rgba(255,222,89,0.4)]" },
    { key: "peace", label: "Peace", emoji: "🌊", color: "bg-peace/20 border-peace/50 text-peace", glow: "shadow-[0_0_20px_rgba(157,229,255,0.4)]" },
    { key: "love", label: "Love", emoji: "🌸", color: "bg-love/20 border-love/50 text-love", glow: "shadow-[0_0_20px_rgba(255,145,145,0.4)]" },
    { key: "reflection", label: "Deep", emoji: "🔮", color: "bg-reflection-blue/20 border-reflection-blue/50 text-reflection-blue", glow: "shadow-[0_0_20px_rgba(59,130,246,0.4)]" },
    { key: "neutral", label: "Calm", emoji: "🌿", color: "bg-white/10 border-white/20 text-white/60", glow: "" },
];
...
export default function NewEntry() {
    ...
    const handleSave = async () => {
        ...
        if (!title || !finalContent) {
            showToast("The most precious moments deserve both a name and a story.", "info");
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                showToast("Memory sealed in the sanctuary", "success");
                router.push("/dashboard");
            } else {
                const data = await res.json();
                showToast(data.message || "Failed to seal your memory.", "error");
            }
        } catch (err) {
            showToast("An error occurred while reaching the sanctuary. Please try again.", "error");
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <span>{loading ? "Sealing..." : "Seal Memory"}</span>
                            </div>
                        </button>
                    </div>
                </header>
                ...
                <main className="premium-card p-8 md:p-32 min-h-[75vh] relative overflow-hidden inner-glow border-white/[0.05]">
                    ...
                    <div className="relative z-10 space-y-16">
                        ...
                        <div className="space-y-6">
                            <label className="text-text-dim text-xs uppercase tracking-[0.6em] font-black ml-1 opacity-50">How do you feel?</label>
                            <div className="flex gap-4 flex-wrap">
                                {MOODS.map((m) => (
                                    <button
                                        ...
                                        className={`flex items-center gap-2.5 px-5 py-2.5 rounded-full border text-[11px] font-black uppercase tracking-widest transition-all duration-300 ${mood === m.key
                                            ? `${m.color} ${m.glow} scale-105`
                                            : "bg-white/[0.03] border-white/10 text-text-dim hover:border-white/20 hover:text-white"
                                            }`}
                                    >
                                        <span>{m.emoji}</span>
                                        <span>{m.label}</span>
                        ...
                        <div className="space-y-8">
                            <label className="text-text-dim text-xs uppercase tracking-[0.6em] font-black ml-1 opacity-50">Moment Title</label>
                        ...
                        <div className="space-y-12">
                            <label className="text-text-dim text-xs uppercase tracking-[0.6em] font-black ml-1 opacity-50">The Narrative</label>
                                ...
                                <AnimatePresence initial={false}>
                                    {blocks.map((block, index) => (
                                        ...
                                        >
                                            ...
                                            {/* Action Bar between blocks - Always visible on mobile, hover-only on desktop */}
                    ...
                    <div className="absolute bottom-16 left-16 flex items-center gap-4 opacity-5 hover:opacity-100 transition-opacity duration-700 pointer-events-none">
                        <Shield size={12} className="text-text-dim" />
                        <span className="text-[9px] font-black uppercase tracking-[0.5em] text-text-dim">Secured Sanctuary • E-Dairy</span>
                    </div>
                </main>
            </div>
        </div>
